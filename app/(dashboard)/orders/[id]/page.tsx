...
import { getMissionById } from "@/lib/api-client";
import type { MissionInfoResponse } from "@/lib/api-client";
import { AlertCircle, Clock, Headphones, Navigation, Map, Route, Store, MapPin, Phone, Pill, Thermometer, ShieldCheck } from "lucide-react";
...
export default function OrderDetailPage({ params }: { params: Promise<OrderDetailParams> }) {
  const { id } = use(params);
  const [order, setOrder] = useState<MissionInfoResponse | null>(null);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            <h2 className="text-white text-3xl font-black tracking-tight">Commande #{order.order?.id || id}</h2>
            <span className="px-3 py-1 rounded-full bg-blue-600/20 text-blue-400 text-xs font-bold border border-blue-500/20 uppercase tracking-wide">
              {order.order?.status || order.status || "Assignée"}
            </span>
          </div>
        ...
        <div className="flex-1 xl:flex-[1.4] flex flex-col gap-6">
          ...
          <div className="rounded-xl border border-slate-800 overflow-hidden shadow-lg relative h-[320px] bg-slate-950 flex items-center justify-center">
            <div className="text-center">
              <Map size={64} className="text-slate-700 mx-auto mb-2" />
              ...
              <div className="pb-8 pt-1">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h4 className="text-white text-lg font-bold">{order.officine?.name || "Pharmacie Locale"}</h4>
                    <p className="text-slate-400 text-sm mt-1">{order.officine?.address || "Adresse de la pharmacie"}</p>
                  </div>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h4 className="text-white text-lg font-bold">
                      {order.patient?.first_name} {order.patient?.last_name || "Client"}
                    </h4>
                    <p className="text-slate-400 text-sm mt-1">{order.patient?.address || "Adresse de livraison"}</p>
                  </div>
                  <button className="p-2 text-slate-500 hover:text-blue-500 transition-colors">
                    <Phone size={20} />
                  </button>
            ...
            <div className="p-5 border-b border-slate-800">
              <h3 className="text-white font-bold text-lg flex items-center justify-between">
                ...
                <span className="text-xs font-medium bg-slate-800 text-slate-300 px-2 py-1 rounded">
                  {order.order?.items?.length || 0} Articles
                </span>
              </h3>
            </div>
                ...
                <div key={idx} className="p-4 flex gap-4 hover:bg-slate-800/50 transition-colors">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <p className="text-white font-medium text-sm">{item.product_name || "Produit"}</p>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-950 border border-slate-800">
                <Thermometer size={20} className="text-blue-400 shrink-0" />
                <div>
